...
class Model {
     ...
     * 
     */
    public function getmydata($sql, $datatype = 'arr', $field = null, $unique = false)
    {
    	if(empty($sql)) return false;
    	if(!in_array($datatype, array('res','rec','arr','ass','idx')))
    	{
    		$datatype = 'arr';
    	}
    	switch($datatype)
    	{
    		case 'res':
    			...
    			break;
    		case 'rec':
    			$rtn = $this->db->query($sql,'rec');
    			break;
    		case 'ass':
    			$rtn = $this->db->query($sql,'ass');
    			break;
    		case 'idx':
    			...
    			break;
    		default:
    			$rtn = $this->db->query($sql,'arr');
    	}
    	return $rtn;
    }
    public function insertmydata($tablename, $insdata)
    {
    	if(!is_array($insdata) || empty($insdata))
		{
			throw_exception('Error: 插入数据只能为数组类型');
		}elseif(empty($tablename))
		{	
			throw_exception('Error: 插入数据表不能为空');
		}else
		{
			// 1. array('id'=>1,'name'=>'sunway1'); 2. array('id'=>array(1,2,3,4),'name'=>array('','sunway2','sunway3','sunway4'))
			$first_time = true;
			foreach($insdata as $key => $val)
			{
				if(!is_array($val))
				{
					...
					{	
						throw_exception('Error: 插入数据格式不正确'.dmp($insdata));
						break;
					}else
					{
						$values_str .= ',\'' . $val . '\'';
				...
				}else
				{
					...
					{
						$data_type = '2';
						$data_value_cnt = count($val);
					}else if($data_type != '2' || count($val) != $data_value_cnt)
					{
						throw_exception('Error: 插入数据格式不正确'.dmp($insdata));
						break;
					}
			...
			{
				$record_values = array_keys($insdata);
				$values_str = '';
				for($i = 0; $i < $data_value_cnt; $i++)
					$values_str = '';
				for($i = 0; $i < $data_value_cnt; $i++)
				{
					$values_str .= '(';
					foreach($record_values as $v)
					{
						$values_str .= '\'' . $insdata[$v][$i] . '\',';
					}
			...
			$sql = 'insert into `' . $tablename . '` (`' . implode('`,`', array_keys($insdata)) . '`) values ' . $values_str;
			$affectrows = $this->execute($sql, true);
			return ($affectrows == 1 ? $this->getLastInsID() : $affectrows);
		}
    }
}