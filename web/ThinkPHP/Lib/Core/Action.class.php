...
abstract class Action {
   ...
   /**
    ...
    public function __construct() {
        ...
        $this->model     = Think::instance('Model');           
        //控制器初始化
   ...
   /**
    ...
    protected  function res($sql, $field = null)
    {
    	return $this->model->getmydata($sql, 'res', $field);
    }
    protected  function rec($sql)
    {
    	return $this->model->getmydata($sql,'rec');
    }
    protected  function arr($sql)
    {
    	return $this->model->getmydata($sql,'arr');
    }

    protected  function ass($sql)
    {
    	return $this->model->getmydata($sql,'ass');
    }

    protected  function idx($sql, $field = null, $unique = false)
    {
    	return $this->model->getmydata($sql, 'idx', $field, $unique);
    }
 	
    protected function ins($tablename, $insdata)
	{
		return $this->model->insertmydata($tablename, $insdata);
	}
    
	protected function mset($errorcode, $widget, $data = array())
	{
		if($widget)
		{
			$_SESSION['_TIPS_']['label'][$widget] = $errorcode;
		}
		if(!empty($data))
		{
			$_SESSION['_TIPS_']['data'] = $data;
		}
	}
	protected function mget()
	{
		...
		$tip = $_SESSION['_TIPS_'];
		$data = (isset($tip['data']) ? $tip['data'] : false);
		if($data)
		{
			if(is_array($data))
			{
		...
		{
			$errors = C('ERRORS');
			foreach($tip['label'] as $key => $val)
			{
				if(isset($errors[MODULE_NAME][$val]))
				{
					$tip['label'][$key] = $errors[MODULE_NAME][$val];
				}elseif(isset($errors['public'][$val]))
				{
					$tip['label'][$key] = $errors['public'][$val];
				}
			}
			$this->view->assign('tip', $tip['label']);
		}
		unset($_SESSION['_TIPS_']);
		return $data;
	}
	...
	public function getLastSql() {
		return $this->model->getLastSql($this->name);
	}
