...
function getvar($vartag, $dval = NULL, $vartype = "string", $darr = array())
{
...
function likefilter($var)
{
	return ($var ? str_replace(array("%","_","'","\\"), array('\%','\_',"''","\\\\"), $var) : $var);
}
function addslashes_deep($var)
{
	if($var)
	{
		$var = is_array($var) ? array_map('addslashes_deep', $var) : addslashes($var);
	}
	return $var;
}
...
function totimestamp($datetime)
{
	...
	$tmp[1] = count($tmp) == 2 ? $tmp[1] : '00:00:00';
