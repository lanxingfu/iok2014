...
class BuyAction extends CommonAction
{
	function index()
	{
		...
		if($keyword)
		{
			...
			switch($item)
			{
				case 'id':
					...
					break;
				case 'title':
					$search = "b.title like '%" . $keyword . "%'";
					break;
				case 'areaid':
					$areaid = $this->arr("SELECT id FROM iok_area WHERE prettyname like '%".$keyword."%'");
					foreach($areaid as $key=>$val)
					{
						$area['areaid'] = $val['id'];
					}
					$data = $this->ass("SELECT * FROM iok_area WHERE id IN ('".$areaid['id']."')");
					dmp($area);die;
					$search = "b.areaid = '".$sellerid."' ";
					break;
				default:
					$search = "b.id = '".$keyword."'";
			}
		...
		$sql = "
			...
			FROM 
				iok_buy b
				left join iok_buyproperty bp on bp.buyid = b.id
			where
				b.enabled = 1" . ($search ? " and $search" : '');
