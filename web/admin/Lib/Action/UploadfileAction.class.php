...
class UploadfileAction extends CommonAction {
    ...
    protected function _upload() {
		...
		//上传路径设置
		$id = $_REQUEST['id'];
		$paths = str_pad($id,8,"0");//以0填充为8位数字符串
		$paths = str_split($paths,3);//每三个一组字符串分割到数组中
		$paths = implode("/",$paths);//把数组元素组合为一个以"/"分隔的字符串。
		$paths = strrev($paths);//反转字符串。
		//$paths = substr_replace($paths,"/",3,3);dmp($paths);
		//$paths = strtok($paths,"/",3);
		//$paths = str_reverse($paths);
		//$upPaths = '/file/upload/product/'.$paths."/".date("Ym")."/".date("d")."/";
		$upPaths = '/file/upload/proof/'.$paths."/";
		$upPaths_up='.'.$upPaths;
		//文件保存路径设置
		...
		$upload->saveRule = time().randcode(6);
        if (!$upload->upload()) {
            //捕获上传异常
	...
	*/
	public function imgdel(){
		...
		$imgurl = $_POST['imgurl'];
		/* 获取图片相对路径 */
		$imgurl_t = explode('/file/upload/proof',$imgurl);
		/* 拼接图片目录路径 */
		$delurl = C("ROOT_PATH").'/file/upload/proof'.$imgurl_t['1'];
		/* 获取图片信息 */
