...
class Image {
     ...
     * @return void
	 * newfun 是用imagecopy生成水印 wuzhijie 2013年11月7日 18:08:03
     */
    static public function water($source, $water, $savename=null, $alpha=80,$newfun=false) {
        //检查文件是否存在
        ...
        if ($sInfo["width"] < $wInfo["width"] || $sInfo['height'] < $wInfo['height'])
            return false;
		// $newfun 生成透明水印图片 使用于我行网商品图片上传水印 wuzhijie 2013年11月7日 18:08:13 
		if($newfun){
			$image_size = getimagesize($source); //上传的图片
			$water_size = getimagesize($water);//水印图片
			//建立图像
			$sCreateFun = "imagecreatefrom" . $sInfo['type'];
			$sImage = $sCreateFun($source);
			$wCreateFun = "imagecreatefrom" . $wInfo['type'];
			$wImage = $wCreateFun($water);
			$x= $image_size[0]/2-$water_size[0]/2;
			$y= $image_size[1]/2-$water_size[1]/2;
			imagecopy($sImage,$wImage,$x,$y,0,0,$water_size[0],$water_size[1]);
