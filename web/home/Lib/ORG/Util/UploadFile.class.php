...
class UploadFile {//类定义开始
    ...
    private function save($file) {
		...
		/* 水印 start 2013年10月11日 18:32:44 wuzhijie 添加水印后生成缩略图 */
		//import("@.ORG.Util.Image");
		//copy($filename,$file['savepath'].'backup_'.$file['savename']);
		//Image::water($filename,'./Public/images/shuiyin.png',null,100,true);
		//Image::water($filename.'.middile'.strtolower($file['extension']), './Public/images/shuiyin2.png',null,100,true);
		/* 水印 end */
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0;$i<$len; $i++) {
					...
					if($i==2){
						Image::water($filename.'.large.'.strtolower($file['extension']), './public/image/public/shuiyin.png',null,100,true);
					}elseif($i==1){
						Image::water($filename.'.middle.'.strtolower($file['extension']), './public/image/public/shuiy.png',null,100,true);
					}
                }
    ...
    public function upload($savePath ='') {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
				...
				/* 2013年10月11日 15:57:04 wuzhijie 修改上传文件扩展名为小写 */
