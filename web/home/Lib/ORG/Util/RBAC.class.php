    ...
    static public function AccessDecision($appName=APP_NAME)
    {
        ...
        if(RBAC::checkAccess()) {
            ...
            if(empty($_SESSION[C('ADMIN_AUTH_KEY')])) {
				...
				$sql = "select * from think_node where name='$module'";
				$data =   $db->query($sql);
				
				$pid = $data[0]['pid'];
				if($pid) {
					$sql = "select * from think_node where id='$pid'";
					$data =   $db->query($sql);
					if(!$data[0]['pid']) {
						$appName = $data[0]['name'];
					}
                ...
                if(!isset($accessList[strtoupper($module)][strtoupper($module)][strtoupper(ACTION_NAME)]) and !isset($accessList[strtoupper($appName)][strtoupper($module)][strtoupper(ACTION_NAME)])) {
