...
function addFav(t) {document.write('<a href="'+window.location.href+'" title="'+document.title+'" rel="sidebar" onclick="window.external.addFavorite(this.href, this.title);return false;" style="background:none;">'+t+'</a>');}
//商品分类
jQuery(document).ready(function (){
	//全部商品分类
	//DQ('dao-title').toggle( function(){Ds('dao-show');}, function(){Dh('dao-show');});
	DQ('dao-title').hover( function(){Ds('dao-show');}, function(){Dh('dao-show');});				
	DQ('dao-show').hover( function(){Ds('dao-show');}, function(){Dh('dao-show');});
	//搜索
	DQ('search-val').toggle( function(){Ds('search-list');}, function(){Dh('search-list');});
	DQ('search-val').hover( function(){Ds('search-list');}, function(){Dh('search-list');});				
	DQ('search-list').hover( function(){Ds('search-list');}, function(){Dh('search-list');});	
});
//划过切换
function Tb(d, t, p, c) {
	for(var i=1; i<=t; i++) {
		if(d == i) {
			Dd(p+'_t_'+i).className = c+'_2'; Ds(p+'_c_'+i);
		} else {
			Dd(p+'_t_'+i).className = c+'_1'; Dh(p+'_c_'+i);
		}
	}
}
/* 公共js函数 */
...
function sortBy (field,sort,act){
	location.href = URL +'/'+ act +"/_order/"+field+"/_sort/"+sort;
}
function cache(){
...
/* 资讯用恢复功能\micheal\2013年5月21日 17:35:29 */
function resume_art(id){
	location.href = URL+"/resume_art/itemid/"+id;
}
function trace(id){
...
function password(id){
	location.href =  "index.php?s=/member/edit/groupid/4/from/1/userid/"+id;
}
...
function foreverdel(id){
	...
	if (window.confirm('确实要永久删除选择项吗？'))
	{
		$.ajax({
			type: "POST",
			url: URL+"/foreverdelete/",
			dataType: "json",
			data: {id:keyValue},
			success: function(msg){
				doDelete(msg.datas,msg.status)
			}
		});
	}
}
...
function goto(){
	...
	location.href='http://'+thisHost+'/Public/login?url='+UrlEncode(thisURL);
}
